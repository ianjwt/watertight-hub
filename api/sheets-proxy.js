...
async function handleCreative(sheets, res, clientConfig) {
  ...
  const adPrefix     = clientConfig?.creative?.adPrefix     || 'Sales_';
  const campaignType = clientConfig?.creative?.campaignType || 'purchase';
  ...
  // Purchase-campaign columns (optional for traffic)
  const colPurch      = findCol(headers, h => h === 'purchases');
  const colROAS       = findCol(headers, h => h.includes('purchase roas'));
  ...
  // Traffic-campaign columns
  const colClicks      = findCol(headers, h => h === 'results');
  const colCTR         = findCol(headers, h => h.includes('ctr'));
  const colCPM         = findCol(headers, h => h.includes('cpm'));
  const colImpressions = findCol(headers, h => h.includes('impression'));

  console.log('[creative] campaignType:', campaignType, '| adPrefix:', adPrefix);
  console.log('[creative] header row:', JSON.stringify(headers));
  console.log('[creative] column indices:', { colAdName, colSpend, colPurch, colROAS, colClicks, colCTR, colCPM, colImpressions });

  if (colAdName === -1 || colSpend === -1) {
    return res.status(500).json({ error: 'Creative sheet column mapping failed — missing ad name or amount spent column' });
  }
  if (campaignType === 'purchase' && colPurch === -1) {
    return res.status(500).json({ error: 'Creative sheet column mapping failed — missing purchases column' });
  }
  ...
  for (const row of rows.slice(1)) {
    ...
    if (!rawName.startsWith(adPrefix)) continue;

    if (campaignType === 'traffic') {
      // ── Traffic row ──────────────────────────────────────────────────────────
      const clicks = parseFloat(row[colClicks] || '0');
      const spend  = parseFloat((String(row[colSpend] || '')).replace(/[$,]/g, '').trim());
      const ctrRaw = colCTR         >= 0 ? parseFloat(row[colCTR]         || '0') : 0;
      const cpmRaw = colCPM         >= 0 ? parseFloat(row[colCPM]         || '0') : 0;
      const impRaw = colImpressions >= 0 ? parseFloat((String(row[colImpressions] || '')).replace(/,/g, '')) : 0;

      if (debugCount < 5) {
        console.log('[creative/traffic] row:', { adName: rawName, clicks, spend, ctrRaw, cpmRaw, impRaw });
        debugCount++;
      }

      if (!clicks || !spend || !Number.isFinite(clicks) || !Number.isFinite(spend)) continue;
      ...
      if (map.has(key)) {
        ...
        e.spend       += spend;
        e.clicks      += clicks;
        e.impressions += impRaw;
        if (ctrRaw > e.bestCTR) {
          e.bestCTR = ctrRaw;
          e.ctr     = ctrRaw;
          e.adName  = parsed.adName;
        }
        if (clicks > e.bestClicks) e.bestClicks = clicks;
      } else {
        map.set(key, {
      ...
      if (debugCount < 5) {
        console.log('[creative/purchase] row:', {
          adName: rawName, spend, purch, roasRaw,
          rawCPA: colCPA >= 0 ? row[colCPA] : '(absent)',
        });
        debugCount++;
      }
  ...
  if (campaignType === 'traffic') {
    const toTrafficResult = (e) => {
      const cpm = (e.impressions > 0)
        ? '$' + (e.spend / e.impressions * 1000).toFixed(2)
        : '$' + e.cpmRaw.toFixed(2);
      return {
    ...
    const rankTraffic = (a, b) => b.clicks - a.clicks;
    const videos = all.filter(e => e.type === 'video').sort(rankTraffic).slice(0, 3).map(toTrafficResult);
    const images = all.filter(e => e.type === 'image').sort(rankTraffic).slice(0, 3).map(toTrafficResult);
    return res.status(200).json({ video: videos, image: images });

  } else {
    const calcROAS = (e) => {
      if (colROAS >= 0 && e.roasWeightSpend > 0) return (e.roasWeightedSum / e.roasWeightSpend).toFixed(2);
      if (colPurchValue >= 0 && e.spend > 0)      return (e.purchaseValue / e.spend).toFixed(2);
      return e.spend > 0 ? '—' : '0.00';
    };
    const toPurchaseResult = (e) => ({
      influencer: e.influencer,
      adName:     e.adName,
    ...
    const rankPurchase = (a, b) => b.purchases - a.purchases || roasNum(b) - roasNum(a);
    const videos = all.filter(e => e.type === 'video').sort(rankPurchase).slice(0, 3).map(toPurchaseResult);
    const images = all.filter(e => e.type === 'image').sort(rankPurchase).slice(0, 3).map(toPurchaseResult);
    return res.status(200).json({ video: videos, image: images });
  }
}
